...
Tests all 22 Indian languages + English for accurate detection
"""
...
def test_language_detection(language_code: str, test_texts: List[str]) -> Dict:
    """Test language detection for a specific language"""
    results = []
    ...
    for text in test_texts:
        try:
            ...
            if response.status_code == 200:
                ...
                is_correct = detected_lang == language_code
                if is_correct:
    ...
    accuracy = (correct_detections / len(test_texts)) * 100 if test_texts else 0
...
def run_comprehensive_test():
    """Run comprehensive language detection test"""
    print("🧪 Starting Comprehensive Language Detection Test")
    print("=" * 60)
    ...
    for lang_code, test_texts in TEST_SAMPLES.items():
        print(f"\n🔍 Testing {lang_code.upper()} ({len(test_texts)} samples)...")
        
        result = test_language_detection(lang_code, test_texts)
        ...
        print(f"   {status} Accuracy: {result['accuracy']:.1f}% ({result['correct_detections']}/{result['total_tests']})")
    ...
    problematic = [(lang, result) for lang, result in all_results.items() if result["accuracy"] < 80]
    if problematic:
        print(f"\n⚠️  PROBLEMATIC LANGUAGES (Accuracy < 80%):")
...
if __name__ == "__main__":
    try:
        ...
        with open("language_detection_test_results.json", "w", encoding="utf-8") as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        
        print(f"\n💾 Detailed results saved to: language_detection_test_results.json")
